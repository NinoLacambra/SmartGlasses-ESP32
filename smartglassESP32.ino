...
#define CHARACTERISTIC_UUID "6d96843a-cbce-4bad-8c6a-693787fa94c2"
...
// Variables changeable during BLE connectser
int _MarginX = 0;
int _MarginY = 0;

int _TouchSensorGPIO = 4;
int _TouchSensorThreshold = 15;
  ...
  public:
    ...
    class BLEConnectState : public BLEServerCallbacks {
      ...
      void onDisconnect(BLEServer* pServer) override {
        ...
        delay(100);
        ESP.restart();  // Bug in BLE: can't reconnect after disconnect, so restart for now
      }
    };
    ...
    class BLEReceiveClass : public BLECharacteristicCallbacks {
      ...
      void onWrite(BLECharacteristic *pCharacteristic) override {
        String sReceived = String(pCharacteristic->getValue().c_str());
        Serial.print("[INFO - BLE] Received: ");
        ...
        else if (sReceived.startsWith("#MX="))  { 
          _MarginX = sReceived.substring(4).toInt(); Serial.println(_MarginX); return; 
        }
        else if (sReceived.startsWith("#MY="))  { 
          _MarginY = sReceived.substring(4).toInt(); Serial.println(_MarginY); return; 
        }
        else if (sReceived.startsWith("#PSI=")) { 
          _PowerSaveInterval = sReceived.substring(5).toInt(); Serial.println(_PowerSaveInterval); return; 
        }
        else if (sReceived.startsWith("#TSG=")) { 
          _TouchSensorGPIO = sReceived.substring(5).toInt(); Serial.println(_TouchSensorGPIO); return; 
        }
        else if (sReceived.startsWith("#TST=")) { 
          _TouchSensorThreshold = sReceived.substring(5).toInt(); Serial.println(_TouchSensorThreshold); return; 
        }
        else if (sReceived.startsWith("#BF="))  { 
          _BatteryVoltageFlat = sReceived.substring(4).toFloat(); Serial.println(_BatteryVoltageFlat); return; 
        }

        // #0,Text,saf....  - #0 - screen select - 0=main
        if (sReceived.startsWith("#")) {
          sReceived = sReceived.substring(1);  // Remove '#'
          clearReceivedData();
    ...
    void init() {
      ...
      pServer->setCallbacks(new BLEReceive::BLEConnectState());
      ...
      pCharacteristic->setCallbacks(new BLEReceive::BLEReceiveClass());
      ...
      String sBleMacAddress = BLEDevice::getAddress().toString();
...
void loop() {
  ...
  if ( getOperatingMode() == WEB_UPDATE ) {
    ...
    Serial.println(touchRead(_TouchSensorGPIO));
  }
}
